  ...
  return {
              ...
              if (opts.parseTags) {
                node.value = node.value.replace(
                  ...
                  (_match, tag) => {
                    ...
                    return `<a href="${base2}/glossary#${slug2}" class="tag-link">${slug2}</a>`;
                  }
                );
              }
            });
    ...
    htmlPlugins() {
      ...
      plugins.push(() => (tree, file) => {
        if (file?.data?.slug !== "glossary") return;
        visit3(tree, "element", (node) => {
          if (node.tagName !== "p") return;
          let firstMeaningful = null;
          for (const child of node.children ?? []) {
            if (child.type === "text" && child.value.trim() === "") continue;
            if (child.type === "element" && child.tagName === "a" && !(child.properties?.className ?? []).includes?.("tag-link") && (child.children ?? []).every((c2) => c2.type !== "text" || c2.value.trim() === "")) {
              continue;
            }
            firstMeaningful = child;
            break;
          }
          if (firstMeaningful?.type === "element" && firstMeaningful.tagName === "a" && Array.isArray(firstMeaningful.properties?.className) && firstMeaningful.properties.className.includes("tag-link")) {
            const href = firstMeaningful.properties?.href ?? "";
            const hashIndex = href.indexOf("#");
            if (hashIndex !== -1) {
              let slug2 = href.slice(hashIndex + 1);
              try {
                slug2 = decodeURIComponent(slug2);
              } catch {
              }
              node.properties = node.properties ?? {};
              if (!node.properties.id) {
                node.properties.id = slug2;
              }
            }
          }
        });
